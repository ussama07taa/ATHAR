...
export default async function ParfumsArabicPage({ searchParams }: PageProps) {
  ...
  // Force the Arabic flag — only fetch Arabic products
  params.set('is_arabic', '1');

  const query = params.toString();

  const [productsRes, filtersRes] = await Promise.all([
    fetch(`${API_URL}/api/products?${query}`, { next: { revalidate: 60 } }),
    fetch(`${API_URL}/api/products/filters?is_arabic=1`, { next: { revalidate: 3600 } }),
  ]);
  ...
  return (
    <Suspense fallback={<ArabicFallback />}>
      <CatalogueContent
